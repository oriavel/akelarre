...
export default class Cueva extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet("vacio", "src/assets/vacio.png", {
      frameWidth: 32,
      frameHeight: 32,
    });
  ...
  create() {
    ...
    //El texto
    this.text = this.add.text(400, 400, "Este es el texto de la ventana", {
      font: "24px Arial",
      fill: "#ffffff",
    });

    //Gráfico adicional
    this.graphicsNombre = this.add.graphics({
      x: this.graphics.x,
    ...
    //Texto adicional
    this.textNombre = this.add.text(400, 400, "AAAAA", {
      fontFamily: "Arial",
      fontSize: "16px",
      color: "#ffffff",
    });
    ...
    const dialogosBruja = [
      ...
      "Por desgracia, tu huida seguirá siendo algo muy difícil de \nconseguir...",
      ...
      "No quiero ni imaginarme cómo harían para que tu sacrificio \nfuera incluso más doloroso, pero seguro que ellas sabrían cómo.",
      "No te preocupes por lo que me pueda pasar a mí cuando se \nenteren de que abrí los portales.",
      "¡Ya iba siendo hora de que les plantase cara a mis hermanas!",
      "Los portales ya están abiertos, ¡mucha suerte! Y que Belzebú \nesté contigo...",
      "¡Ah, espera! Casi se me olvida, por favor, no entres en el \nPortal rojo del centro hasta que no tengas las otras piezas",
      "Por ese portal solo puede entrar Sorgina, \nmi hermana mayor y la más malvada.",
      "Ahí guarda la última pieza de llave, no confía en nosotras \npara salvaguardarla...",
    ...
    const dialogosMotos = [
      ...
      "Me dijeron que iban a ofrecerme a su Dios, pero creo que al \nfinal pude convencerlas de que estaba demasiado bueno para eso.",
      "Desde ese momento me han dejado aquí en una esquina, no sé \na qué esperan para dejarme ir ja ja ja ja...",
    ...
    const dialogosGato = [
      ...
      "Tú eres la siguiente en la lista. Veo que eres \nmuy joven, pobrecita, qué poco has vivido...",
      "JA JA JA JA JA, ¡qué pringada!",
      "Te aviso de antemano que no hay escapatoria, la salida\nestá bloqueada por una puerta mágica que necesita 3 llaves.",
      "¡Y no pienso decirte dónde están! JA JA JA JA",
    ...
    const dialogosCaldero = [
      "Supongo que con esto hacen sus pociones, qué estereotípico",
      "Está burbujeando, me pregunto qué efecto \ntendría si lo bebo",
      "Huele fatal...",
    ];

    const dialogoCartel1 = [
      "Pone '¡Os tengo dicho que a este portal solo entro yo!,\nHe puesto una alarma de lo harta que me tenéis.'",
